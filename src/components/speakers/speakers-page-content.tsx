...
export function SpeakersPageContent() {
  const t = useTranslations("speakerShowcase");
      ...
      <div className="mx-auto max-w-6xl space-y-8 px-4 py-4 md:px-6 md:py-5">
        <section className="overflow-hidden rounded-[2rem] bg-[radial-gradient(circle_at_65%_20%,rgba(255,255,255,0.08),transparent_18%),radial-gradient(circle_at_85%_10%,rgba(125,93,255,0.22),transparent_25%),linear-gradient(120deg,#20124d_0%,#2a145d_35%,#131a34_100%)] px-8 py-14 text-white shadow-[0_18px_40px_rgba(15,23,42,0.18)]">
          <div className="max-w-xl space-y-4">
            <h1 className="m-0 max-w-lg text-[3.3rem] font-semibold leading-[1.02] tracking-[-0.05em]">{t("heading")}</h1>
            <p className="m-0 max-w-lg text-base leading-7 text-white/75">{t("intro")}</p>
          </div>
        </section>
        ...
        <section className="space-y-5">
          <div className="flex flex-wrap items-center justify-between gap-4">
            ...
            <div className="text-sm font-semibold text-slate-400">{t("sortedAZ")}</div>
          </div>
              ...
              >
                ...
                <div className="space-y-2 px-1 pb-1">
                  <div className="space-y-1">
                    ...
                    <p className="m-0 text-sm font-medium text-[#6f95e8]">{t(`cards.${speaker.id}.role`)}</p>
                  ...
                  <div className="flex items-center justify-between pt-2">
                    ...
                    {speaker.selected ? (
                      <span className="rounded-full bg-[#e8f0ff] px-3 py-1 text-xs font-semibold text-[#4d8cff]">{t("selected")}</span>
                    ) : (
        ...
        <section className="rounded-[1.8rem] bg-white p-5 shadow-[0_1px_0_rgba(15,23,42,0.05)]">
          <div className="grid gap-6 md:grid-cols-[18rem_minmax(0,1fr)]">
            ...
            <div className="space-y-5">
              ...
              <div className="space-y-2">
                <p className="m-0 text-xs font-semibold uppercase tracking-[0.18em] text-slate-400">{t("about")}</p>
                <p className="m-0 max-w-3xl text-sm leading-7 text-slate-500">{t("featured.about")}</p>
              </div>

              <div className="space-y-3">
                <p className="m-0 text-xs font-semibold uppercase tracking-[0.18em] text-slate-400">{t("scheduledSessions")}</p>
                <div className="space-y-3">
